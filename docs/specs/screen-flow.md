...
- 최초 조우한 포켓몬은 지역 도감에 체크한다.
- 이브이와 메타몽의 지역 도감 체크는 [지역별 조우 정책](./region-encounters.md)의 공통 규칙을 따른다.
- 조우 성공 시 문제 풀이와 몬스터볼 포획 플로우로 진입한다.
- 조우 문제 화면은 URL의 지역·포켓몬 식별자로 복원하며, 지역 출현 목록에 없는 조합은 탐험 지도로 돌려보낸다.
- 조우 문제는 기존 JavaScript 문제 목록과 현재 문제 선택 상태를 재사용한다. 몬스터볼 수량과 포획 결과 상태는 다음 포획 단계에서 별도로 관리한다.
...
2. 프리저, 썬더, 파이어가 지정 지역에서 해금
3. 전설의 새 3종 등록 → 현대 도시에서 뮤츠 해금
4. 뮤츠까지 150종 등록 → 뮤 돌발 이벤트 팝업 노출
5. 뮤 졸업 → 엔딩
