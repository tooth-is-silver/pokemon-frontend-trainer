...
- [x] 조우 성공 결과 UI 표시
- [x] 지역별 출현 포켓몬 146종 배치와 희귀도 확률 수치화
- [x] 이브이·메타몽 공통 출현 및 지역 도감 체크 정책 정의
- [x] 전설 포켓몬 지역 배치와 해금 순서 정의
- [x] 지역별 출현 풀을 코드 데이터로 정의
- [x] 조우 기록을 서버에 저장하고 지역 도감에 연결
- [x] 전체 포켓몬 랜덤 선택을 지역별 확률 추첨으로 교체
- [x] 조우 성공 시 문제 풀이 진입 연결
...
> 포획형 전환 전 구현 이력이다. 목표 정책은 지역별 전설 조우 방식으로 교체됐다.
